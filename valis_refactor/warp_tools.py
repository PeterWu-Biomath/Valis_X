...
from _valtils import get_name, levenshtein_d, print_warning, get_ncpus_available
...
def get_ref_img_idx(img_f_list, ref_img_name=None):
    ...
    if ref_img_name is None:
        ...
        ref_img_name = get_name(os.path.split(ref_img_name)[1])
        img_names = [get_name(f).lower() for f in img_f_list]
        ref_name_lower = ref_img_name.lower()

        try:
            # Check for exact matches
            ref_img_idx = img_names.index(ref_name_lower)
        except ValueError:
            # Get closest match
            string_d = [levenshtein_d(ref_name_lower, img_names[i]) for i in range(len(img_names))]
            ref_img_idx = np.argmin(string_d)
            warning_msg = (f"No files in `img_f_list` match exactly match {ref_img_name}. "
                           f"Returning closest match, which is {get_name(img_f_list[ref_img_idx])}")
...
def stitch_tiles(tile_list, tile_bboxes, nrow, ncol, overlap):
    ...
    for i in range(nrow):
        ...
        for j in range(1, ncol):
            ...
            #import pdb
            #pdb.set_trace()
            #print(f"Merging tiles {tile_idx} and {left_idx},right_tile {right_tile}")
            #print(f"{i},{j}, right_tile {right_tile},row_mosaic {row_mosaic},offset {offset},overlap {overlap}")
            try:
                row_mosaic = row_mosaic.merge(right_tile, "horizontal", offset, 0, mblend=overlap)
            except Exception as e:
                print(f"Error type: {type(e)}")
                print(f"Error message: {e}")
                print(f"Error merging tiles {tile_idx} and {left_idx},right_tile {right_tile},row_mosaic {row_mosaic},offset {offset},overlap {overlap}")
                quit()
    ...
    for i in range(1, nrow):
        ...
        try:
            ...
            print(f"Error type: {type(e)}")
            print(f"Error message: {e}")
            print(f"Error merging tiles {i} and {i-1},bottom {bottom},stitched {stitched},v_offset {v_offset},overlap {overlap}")
            quit()
...
def rotate(
    ...
    Copied and pasted from scikit image, but modified to return transform
    and enusre rotated image has same datatype as original when `preserve_range=True`
...
def warp_img(img, M=None, bk_dxdy=None, out_shape_rc=None,
             ...
             is_ref=False,dst_f=None,debug=False):
    ...
    if do_rigid:
        ...
        if is_ref:
            ...
            with open(out_path,"w") as w:
                w.write(f"{bbox_xywh} {-tx} {-ty}")
            return [bbox_xywh[0]+tx,bbox_xywh[1]+ty]
    ...
    if bbox_xywh is not None:
        try:
            ...
            width_padding = bbox_xywh[2]+bbox_xywh[0] - warped.width
            height_padding = bbox_xywh[3]+bbox_xywh[1] - warped.height
            if width_padding > 0:
                bbox_xywh[2] += -width_padding
            if height_padding > 0:
                bbox_xywh[3] += -height_padding
            warped = warped.extract_area(*bbox_xywh)
            
            if width_padding > 0:
                warped = warped.embed(0, 0, warped.width+width_padding, warped.height)
            if height_padding > 0:
                warped = warped.embed(0, 0, warped.width, warped.height+height_padding)
...
#     print_warning("Looking for folds", None, rgb=Fore.YELLOW)
...
#         print_warning("Removing folds using regularizaation", None, rgb=Fore.YELLOW)
